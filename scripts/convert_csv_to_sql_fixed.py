#!/usr/bin/env python3
"""
CSV 파일을 SQL INSERT 문으로 변환 (payment_type 포함)
실행: python scripts/convert_csv_to_sql_fixed.py
"""
import csv
...
def get_payment_type(method):
    """결제 방법에 따른 payment_type 결정"""
    if method in ["card", "카드"]:
        return "one_time"  # 카드는 보통 일회성
    else:
        return "one_time"  # 기본값
...
def convert_csv_to_sql():
    ...
    print("🚀 CSV를 SQL INSERT 문으로 변환 (payment_type 포함)")
    print("=" * 50)
    
    # CSV 파일 경로
    csv_path = Path("backend/seed/payments_real.csv")
    sql_path = Path("sql/insert_payments_fixed.sql")
    ...
    with open(csv_path, "r", encoding="utf-8") as csvfile:
        ...
        with open(sql_path, "w", encoding="utf-8") as sqlfile:
            # 헤더 작성
            sqlfile.write("-- 로컬 결제 데이터 마이그레이션 (payment_type 포함)\n")
            ...
            for row in reader:
                ...
                if batch_count % 10 == 0:
                    if batch_count > 0:
                        sqlfile.write(";\n\n")
                    sqlfile.write("INSERT INTO temp_payments (customer_id, payment_date, amount, payment_method, payment_type, payment_status, created_at) VALUES\n")
                else:
                    sqlfile.write(",\n")
                
                # 값 작성
                payment_method = convert_payment_method(row['payment_method'])
                payment_type = get_payment_type(row['payment_method'])
                created_at = row['created_at'] if row['created_at'] else 'NOW()'
                
                sqlfile.write(f"({row['customer_id']}, '{row['payment_date']}', {row['amount']}, '{payment_method}', '{payment_type}', 'completed', '{created_at}')")
            ...
            sqlfile.write("""INSERT INTO payments (customer_id, payment_date, amount, payment_method, payment_type, payment_status, created_at)
