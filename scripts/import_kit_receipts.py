#!/usr/bin/env python3
"""
검사키트 수령(kit_receipts) 데이터를 프로덕션 DB에 시드하는 스크립트
    ...
    for idx, row in df.iterrows():
        try:
            ...
            kit_data = {
                ...
                'kit_serial': serial,
                'received_date': parse_date(row.get('키트수령일', row.get('received_date', row.get('kit_received_date')))),
                'result_date': parse_date(row.get('결과지수령일', row.get('result_date', row.get('result_received_date')))),
                'delivered_date': parse_date(row.get('결과지전달일', row.get('delivered_date', row.get('result_delivered_date')))),
