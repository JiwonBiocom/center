#!/usr/bin/env python3
"""
데이터베이스의 2025년 5월 결제 데이터 확인 - 수정 버전
"""
...
def check_may_2025_payments():
    ...
    try:
        ...
        if payments:
            ...
            print("\n상세 내역:")
            print("-" * 100)
            print(f"{'ID':>5} {'날짜':^12} {'고객명':^15} {'금액':>12} {'담당자':^10}")
            print("-" * 100)
            
            for p in payments[:10]:  # 처음 10건만 출력
                date_str = p.payment_date.strftime('%Y-%m-%d')
                customer_name = p.customer_name[:15] if p.customer_name else 'Unknown'
                print(f"{p.payment_id:>5} {date_str:^12} {customer_name:^15} {float(p.amount):>12,.0f} {p.payment_staff:^10}")
            
            if len(payments) > 10:
                print(f"... 그 외 {len(payments) - 10}건")
                
            print("-" * 100)
            print(f"{'합계':>45} {total_amount:>12,.0f}원")
            ...
            if total_amount == expected_total:
                ...
                date_totals = date_result.fetchall()
                
                print("\n날짜별 결제 현황:")
                print("-" * 50)
                for dt in date_totals:
                    print(f"{dt.payment_date}: {dt.count}건, {float(dt.total):,.0f}원")
