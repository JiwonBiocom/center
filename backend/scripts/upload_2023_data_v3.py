#!/usr/bin/env python3
"""2023년 결제 데이터 업로드 스크립트 v3 - 고객 생성/조회 포함"""
...
def get_or_create_customer(customer_name):
    ...
    response = requests.get(
        f"{API_BASE_URL}/customers",  # trailing slash 제거
    ...
    customer_data = {
        "name": customer_name,
        "phone": "",  # 23년 데이터에는 전화번호가 없음
        "email": "",
        "gender": "other",
        "birth_date": None,
        "address": "",
        "notes": "2023년 결제 데이터에서 자동 생성"
    }
    ...
    if response.status_code in [200, 201]:
        ...
        print(f"  고객 생성 실패: {customer_name} - {response.status_code}")
...
def extract_payment_data(df, sheet_name):
    ...
    for _, row in df.iterrows():
        try:
            ...
            if "계좌이체" in str(row.values):
                payment_method = "transfer"
            elif "현금" in str(row.values):
                payment_method = "cash"
            ...
            payment = {
                ...
                "payment_staff": "",
                "notes": f"{product_name}. " + " / ".join(memo_parts) if memo_parts else product_name
            }
...
def main():
    ...
    for payment in all_payments:
        try:
            ...
            response = requests.post(
                f"{API_BASE_URL}/payments",  # trailing slash 제거
                json=payment_data
            )
            ...
            if response.status_code in [200, 201]:
                ...
                if error_count <= 3:  # 처음 몇 개 에러만 출력
                    print(f"  에러: {customer_name} - {response.status_code}")
