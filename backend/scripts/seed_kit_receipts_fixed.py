    ...
    with Session(engine) as session:
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # 새 키트 수령 정보 생성 - 실제 모델 구조에 맞춤
                kit_receipt = KitReceipt(
                    customer_id=customer_id,
                    kit_type=row.get('키트종류', row.get('kit_type', '장내미생물')),
                    serial_number=serial,  # 모델에서는 serial_number 사용
                    receipt_date=parse_date(row.get('키트수령일', row.get('received_date'))),
                    result_received_date=parse_date(row.get('결과지수령일', row.get('result_date'))),
                    result_delivered_date=parse_date(row.get('결과지전달일', row.get('delivered_date')))
                )
        ...
        samples = session.execute(text("""
            SELECT c.name, c.phone, kr.kit_type, kr.serial_number, kr.receipt_date 
            FROM kit_receipts kr
            JOIN customers c ON kr.customer_id = c.customer_id
            ORDER BY kr.kit_id DESC 
            LIMIT 5
        """)).fetchall()
